...
pygame.display.set_caption("Screen touch Visualiser v1.2.0")
pygame.scrap.init()
...
butterflyImage = os.path.join(THIS_FOLDER, 'Resources\\butterfly.png')
...
crossPNG = os.path.join(THIS_FOLDER, 'Resources\\cross.png')
pinkCirclePNG = os.path.join(THIS_FOLDER, 'Resources\\pinkCircle.png')
appenderPATH = os.path.join(THIS_FOLDER, 'appender.py')
# ----------------------
...
butterfly = pygame.image.load(butterflyImage).convert_alpha()
pinkCircle = pygame.image.load(pinkCirclePNG).convert_alpha()
...
lblTimeBar = font.render(str("TIME SELECTOR"), True, black)
lblPercent = font.render(str("100 %"), True, black)
...
# GRAPHIC : Time Selector
def timeSelection():
  #INPUT BAR CONTROLLING
  pygame.draw.rect(ecran, whiteVisualiser, (700, 450, 400, 7))
  pygame.draw.rect(ecran, black, (xTimeBar, yTimeBar, 9, 30))

  ecran.blit(lblTimeBar, (700, 400))
  ecran.blit(lblPercent, (1000, 400))

# CONTROLLER : Bound the UI bar with the nulber of fps
def moovingBarSpeed(xSpeed, ySpeed, moovingIsOk, myTempSpeed):
    if pygame.mouse.get_pressed() == (0,0,0):
      moovingIsOk = False

    if clickOnMe:
      if pygame.mouse.get_pressed() == (1,0,0):
          if mousePos[0] > xSpeed - 3:
            if mousePos[0] < xSpeed + 9:
              if mousePos[1] > ySpeed:
                if mousePos[1] < ySpeed + 30:
                  moovingIsOk = True
    ...
    myTempSpeed = (xSpeed / 2) - 330

    return xSpeed, moovingIsOk, myTempSpeed
...
def fileOpenning(part1, finalListOfData, count, doesMyFileExist, loopData, perkCount, tempLists):
  ...
  if doesMyFileExist:
    ...
    if max47Code > 1:
        ...
        with open(appenderPATH, "w+") as fAppender:
...
def drawLine(coordinatesOfLayer, validState, loopData, makeAFor, IfinishedToDraw, oldLoopData, oldPerkCount, timeSequense):
    ...
    for u in range(0, makeAFor):
      ...
      if validState:
        ...
        if newFinger:
          ecran.blit(butterfly, (endx - 12, endy - 12))

        # Draw Begin Of Lines
        if newFingerBegin:
          ecran.blit(pinkCircle, (int(startx - 5), int(starty - 5)))
          newFingerBegin = False
...
#-----------------------------------------START LOOP--------------------------------------------
while run:
  timeSequense = int((xTimeBar - 700) / 4)

  # NEED To reset the FPS Label
  lblNumberOfFps = font.render(str(myTempSpeed), True, black)
  lblPercent = font.render(str(timeSequense) + " %", True, black)
  ...
  if iPressedPartialButton:
    numberOfFPSArea()
    xSpeedBar, moovingIsOk, myTempSpeed = moovingBarSpeed(xSpeedBar, ySpeedBar, moovingIsOk, myTempSpeed)
    if not(focus):
  ...
  if iPressedTotalButton:
    timeSelection()
    xTimeBar, moovingIsOk, myTempSpeed = moovingBarSpeed(xTimeBar, yTimeBar, moovingIsOk, myTempSpeed)
  ...
  if not(iPressedMyStopButton):  
    if iAmOnMultipleTouch == False:
      loopData, makeAFor, IfinishedToDraw, oldLoopData, oldPerkCount, lblPosX, lblPosY = drawLine(coordinatesOfLayer, validState, loopData, makeAFor, IfinishedToDraw, oldLoopData, oldPerkCount, timeSequense)
    else:
      if iPressedMyButton:
        bigList = []
      if iPressedTotalButton:
        if len(bigList) < (lenOfMyList[0] + lenOfMyList[1] + lenOfMyList[2] + lenOfMyList[3] + lenOfMyList[4]):
          bigList += myFinalList
        if len(bigList) >= (lenOfMyList[0] + lenOfMyList[1] + lenOfMyList[2] + lenOfMyList[3] + lenOfMyList[4]):
          loopData, makeAFor, IfinishedToDraw, oldLoopData, oldPerkCount, lblPosX, lblPosY = drawLine(bigList, validState, loopData, makeAFor, IfinishedToDraw, oldLoopData, oldPerkCount, timeSequense)
      else:
        loopData, makeAFor, IfinishedToDraw, oldLoopData, oldPerkCount, lblPosX, lblPosY = drawLine(myFinalList, validState, loopData, makeAFor, IfinishedToDraw, oldLoopData, oldPerkCount, timeSequense)
  else:
    bigList = []
    coordinatesOfLayer = []
    tempLists = []
